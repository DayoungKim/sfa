...
#confusing names
#slice : sfa slice matched with openstack slice member user
#sliver : sfa sliver matched with openstack heatstack object 
#server : (no sfa name) matched with openstack nova server
#instance : matched with openstack heatstack object
class OpenstackDriver(Driver):
    ...
    def register_federation(self, user_hrn, slice_hrn, keys, email=None):
        ...
        heatstackowner_role = self.shell.auth_manager.roles.find(name='heat_stack_owmer')
        if self.shell.auth_manager.roles.roles_for_user(user, tenant).count(member_role) == 0:
            self.shell.auth_manager.roles.add_user_role(user, member_role, tenant)
            self.shell.auth_manager.roles.add_user_role(user, heatstackowner_role, tenant)
...
#            key_name = self.shell.compute_manager.keypairs.get(name).name
        ...
        for sliver in slivers:
            component_id = sliver.description.get('component_id')
            sliver_id = OSXrn(name=('koren'+'.'+ sliver.stack_name), id=sliver.id, type='node+openstack').get_urn()
...
#                  'geni_expires': datetime_to_string(utcparse(time.time())) })
    ...
    def delete(self, urns, options=None):
        ...
        user_name = xrn.get_authority_hrn() + '.' + xrn.leaf.split('-')[0]
        tenant_name = OSXrn(xrn=urns[0], type='slice').get_hrn()
        self.shell.orchest_manager.connect(username=user_name, tenant=tenant_name, 
                                           password=user_name)
        ...
        id_set = set()
        for instance in instances:
            sliver_id = OSXrn(name=('koren'+'.'+ instance.stack_name), 
